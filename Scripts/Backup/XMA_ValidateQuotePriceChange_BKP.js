...
async function activateQuoteAsync(executionContext){
    ...
    await model.invokeActivateQuoteRibbonAction(); 
    //await model.activateQuoteAsync();    
};
...
async function checkQuotePriceChanges(executionContext){
    ...
    let result = await Xrm.WebApi.retrieveMultipleRecords("quotedetail", `?$select=priceperunit,xma_issellpricemanuallychanged,igl_originalpeprice,igl_productcode,quotedetailid&$filter=_quoteid_value eq ${quoteId}`);        
    if (result != null){
        result.entities.forEach(function (entity) {
            if ((entity.priceperunit != entity.igl_originalpeprice) && (entity.xma_pricechanged != 2 && entity.xma_issellpricemanuallychanged == false)) {            
                itemPriceChanged +=  "The sell price for the item " +entity.igl_productcode+ " has been changed in the pricing engine [New price : £" +entity.priceperunit+ " - Original price : £"+entity.igl_originalpeprice+"]. \n";
            }            
        });
    } 
